...
from .deployment_files import (DOCKER_COMPOSE_FILENAME,
                               DOCKER_COMPOSE_OVERRIDE_FILENAME, ConfigFile,
                               DockerComposeFile, EditableDockerCompose)
...
class DeploymentFolder(Deployable):
  ...
  def __init__(
    ...
    if not os.path.isfile(self._working_config.filepath) and not os.path.isfile(self._template_config.filepath):
      raise FileNotFoundError(f"either {self._working_config.filepath} or "
                              f"{self._template_config.filepath} should ex"
                              "ist, none found")
