...
class TestDeploymentGeneric(FileSystemTestCase):
  ...
  def check_zip(self, out_ref_path, zip_dir):
    ...
    with zipfile.ZipFile(os.path.join(zip_dir, zip_file), "r", zipfile.ZIP_DEFLATED) as zip_archive:
      self.assertListEqual(sorted(zip_archive.namelist()), sorted(list_relative_files(out_ref_path)))
